...
providing specs for each argument.
...
`llm-tool-collection-get-all' and `llm-tool-collection-get-category'."
  ...
  (let* ((arg-syms (mapcar #'car args))
         (arg-specs (mapcar (lambda (arg)
                              `(:name ,(symbol-name (car arg)) ,@(cdr arg)))
                            args))
         (sym (llm-tool-collection--name-to-symbol name))
